...
export function LandingPage() {
  ...
  return (
    <div className="w-full max-w-none m-0 p-0 animate-fade-in">
      {/* Hero Section */}
        <section className="relative w-full bg-primary-950 bg-[url('/hero.jpg')] bg-cover bg-center bg-no-repeat text-white md:min-h-screen hero-section will-change-scroll">
            ...
            <div className="grid grid-cols-2 gap-4 animate-fade-in-up">
              {[
                ...
                { label: 'Citoyens actifs', value: stats.citizens, icon: Users, color: 'from-coral-400 to-coral-600' },
                  ...
                  <div key={s.label} className="rounded-2xl bg-white/5 force-android-blur border border-white/40 p-4 md:p-6 hover:bg-white/10 transition-colors" style={{ animationDelay: `${i * 100}ms` }}>
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            <div key={f.title} className="card p-6 hover:shadow-natural-hover transition-transform transition-shadow hover:-translate-y-0.5 group">
          ...
          {loading ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {recentSignalements.map((sig) => (
                <Link key={sig.id} to={`/signalements/${sig.id}`} className="card p-5 hover:shadow-natural-hover transition-transform transition-shadow hover:-translate-y-0.5">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {recentArticles.map((a) => (
                  <Link key={a.id} to={`/education/${a.slug}`} className="card p-5 flex gap-4 hover:shadow-natural-hover transition-shadow">
                    {a.image_url ? (
