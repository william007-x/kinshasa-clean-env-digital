      ...
      <div className="hidden lg:flex lg:w-1/2 gradient-hero relative overflow-hidden">
        ...
        <div className="relative flex flex-col justify-center px-16 text-white">
          ...
          <ul className="space-y-3 text-sand-200">
            <li className="flex items-center gap-3"><Check className="h-5 w-5 text-primary-300" /> Signalez en temps réel les incidents environnementaux</li>
            <li className="flex items-center gap-3"><Check className="h-5 w-5 text-primary-300" /> Suivez la cartographie des 24 communes</li>
            <li className="flex items-center gap-3"><Check className="h-5 w-5 text-primary-300" /> Accédez aux ressources éducatives</li>
          </ul>
        </div>
      </div>
        ...
        <div className="w-full max-w-md animate-fade-in-up">
          <div className="lg:hidden mb-8">
            <Link to="/" className="flex items-center gap-2.5">
              ...
              <span className="font-display text-xl font-extrabold text-primary-800">KinshasaEco</span>
            </Link>
          </div>
          <h1 className="font-display text-3xl font-bold text-primary-900">{title}</h1>
          <p className="mt-2 text-primary-500">{subtitle}</p>
          <div className="mt-8">{children}</div>
          <div className="mt-6 text-center text-sm text-primary-500">{footer}</div>
        </div>
      </div>
    </div>
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link to="/mot-de-passe-oublie" className="text-xs font-medium text-primary-600 hover:text-primary-700">Mot de passe oublié ?</Link>
          </div>
          <div className="relative">
    ...
    return (
      ...
      >
        <div className="rounded-2xl bg-primary-50 shadow-natural p-6 text-center">
          ...
          <h3 className="font-display font-semibold text-primary-900">Bienvenue, {fullName} !</h3>
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-primary-400" />
            ...
            {ROLE_OPTIONS.map((r) => (
              <button
                ...
                className={classNames(
                  ...
                  role === r.value ? 'border-primary-500 bg-primary-50 ring-2 ring-primary-200' : 'border-sand-300 hover:border-primary-300'
                )}
              >
                <p className="text-sm font-semibold text-primary-900">{ROLE_LABELS[r.value]}</p>
    ...
    >
      {sent ? (
        <div className="rounded-2xl bg-primary-50 shadow-natural p-6 text-center">
          ...
          <h3 className="font-display font-semibold text-primary-900">Email envoyé</h3>
          <p className="mt-2 text-sm text-primary-600">
