    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {/* Role distribution */}
          <Card className="p-6">
            <h3 className="font-display font-semibold text-primary-900 mb-4">Répartition par rôle</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
              {(Object.keys(ROLE_LABELS) as UserRole[]).map((role) => {
                const count = users.filter((u) => u.role === role).length;
                return (
                  <div key={role} className="text-center p-3 rounded-xl bg-sand-50">
                    <p className="font-display text-2xl font-bold text-primary-900">{count}</p>
                    <p className="text-xs text-primary-500 mt-1">{ROLE_LABELS[role]}</p>
                  </div>
                );
              })}
            </div>
          ...
          <Card className="overflow-hidden">
            <div className="px-6 py-4 border-b border-sand-200">
              <h3 className="font-display font-semibold text-primary-900">Signalements en attente de modération</h3>
            </div>
            {pendingSignalements.length === 0 ? (
              <div className="px-6 py-8 text-center text-primary-400 text-sm">Aucun signalement en attente.</div>
            ) : (
                ...
                {pendingSignalements.slice(0, 5).map((sig) => (
                  <div key={sig.id} className="flex items-center gap-4 px-6 py-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-primary-400">{sig.profiles?.full_name} · {timeAgo(sig.created_at)}</p>
                    </div>
                    <button onClick={() => updateSignalementStatus(sig.id, 'en_cours')} className="btn-ghost text-xs"><CheckCircle2 className="h-4 w-4 text-primary-600" /> Valider</button>
        ...
        <>
          ...
          <Card className="overflow-hidden">
            <div className="divide-y divide-sand-100">
              {filteredUsers.map((u) => (
                <div key={u.id} className="flex items-center gap-4 px-5 py-4 hover:bg-sand-50">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-100 text-primary-700 font-semibold">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-primary-900 truncate">{u.full_name}</p>
                      <p className="text-xs text-primary-400">{timeAgo(u.created_at)}</p>
                    </div>
              ...
              {filteredUsers.length === 0 && <div className="px-5 py-8 text-center text-primary-400 text-sm">Aucun utilisateur trouvé.</div>}
            </div>
          </Card>
        </>
      ) : tab === 'signalements' ? (
          ...
          <div className="divide-y divide-sand-100">
            {signalements.map((sig) => (
              <div key={sig.id} className="flex items-center gap-4 px-5 py-4 hover:bg-sand-50">
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-primary-900 text-sm truncate">{sig.title}</p>
                  <p className="text-xs text-primary-400">{sig.profiles?.full_name} · {sig.communes?.name ?? 'N/A'} · {timeAgo(sig.created_at)}</p>
                </div>
            ...
            {signalements.length === 0 && <div className="px-5 py-8 text-center text-primary-400 text-sm">Aucun signalement.</div>}
          </div>
        </Card>
      ) : tab === 'articles' ? (
        <Card className="overflow-hidden">
          <div className="divide-y divide-sand-100">
            {articles.map((a) => (
              <div key={a.id} className="flex items-center gap-4 px-5 py-4 hover:bg-sand-50">
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-primary-900 truncate">{a.title}</p>
                  <p className="text-xs text-primary-400">{a.profiles?.full_name} · {ARTICLE_CATEGORY_LABELS[a.category]} · {a.views} vues</p>
                </div>
                ...
                <button onClick={() => toggleArticlePublished(a)} className="btn-ghost text-sm">
                  {a.published ? <XCircle className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4 text-primary-600" />}
            ...
            {articles.length === 0 && <div className="px-5 py-8 text-center text-primary-400 text-sm">Aucun article.</div>}
          </div>
        </Card>
      ) : (
        <Card className="overflow-hidden">
          <div className="divide-y divide-sand-100">
            {auditLogs.map((log) => (
              <div key={log.id} className="flex items-start gap-4 px-5 py-4">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-sand-100 text-primary-500">
                  <Activity className="h-4 w-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-primary-900">
                    <span className="font-semibold">{log.action}</span> sur <span className="text-primary-600">{log.entity_type}</span>
                  </p>
                  ...
                  {Object.keys(log.details).length > 0 && (
                    <pre className="mt-1.5 text-xs text-primary-500 bg-sand-50 rounded-lg p-2 overflow-x-auto">{JSON.stringify(log.details, null, 2)}</pre>
                  )}
                </div>
              </div>
            ))}
            {auditLogs.length === 0 && <div className="px-5 py-8 text-center text-primary-400 text-sm">Aucune action enregistrée.</div>}
          </div>
        </Card>
      )}
      ...
      <Modal open={!!editUser} onClose={() => setEditUser(null)} title="Modifier le rôle">
        {editUser && (
          <div className="space-y-4">
            <div>
              <p className="text-sm text-primary-600">Utilisateur: <span className="font-semibold text-primary-900">{editUser.full_name}</span></p>
              <p className="text-xs text-primary-400 mt-0.5">Rôle actuel: {ROLE_LABELS[editUser.role]}</p>
            </div>
            <div>
